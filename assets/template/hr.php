                  ...
                  <div job=\"" . $row['jid'] . "\" class=\"results_item_content\">
                    ...
                    if($winners_sql->fetchColumn() > 0) {
                        ...
                        foreach($winners_sql as $winner) {
                            echo "<tr><td>$iter. {$winner['worker_name']}</td><td>{$winner['seniority_date']}</td><td>{$winner['worker_dept']}</td><td style='text-align: center !important;'><strong>{$winner['preference']}</strong></td><td><a href=\"?bid_id={$winner['bid']}&action=confirm_winner\" onclick=\"confirmWinner('$winner['bid']');\" class=\"low_pro button active\">Confirm winner</a></td></tr>";
                            $iter++;
                        }
                        echo "</table>";
                    }
