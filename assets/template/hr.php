          ...
          <ul class="results_list">
            ...
            <?php
              ...
              if(isset($_GET['bid_id']) && isset($_GET['action']) && $_GET['action'] == "confirm_winner") {
                ...
                $_query_text = "INSERT INTO confirmed_winners VALUES (:new_department, :new_job_name, :old_department, :old_job_name, :eid, :worker_name)";
                  ...
                  <div job=\"" . $row['jid'] . "\" class=\"results_item_content\">
                    ...
                    if($winners_sql->fetchColumn() > 0) {
                        ...
                        foreach($winners_sql as $winner) {
                            echo "<tr><td>$iter. {$winner['worker_name']}</td><td>{$winner['seniority_date']}</td><td>{$winner['worker_dept']}</td><td style='text-align: center !important;'><strong>{$winner['preference']}</strong></td><td><a onclick=\"confirmWinner('{$winner['bid']}');\" class=\"low_pro button active\">Confirm winner</a></td></tr>";
                            $iter++;
                        }
                        echo "</table>";
                    }
